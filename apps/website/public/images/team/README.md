# Team Image Index

This folder holds team portrait assets used across the website.

## Current Contents

- `arnold-garcia.webp` - Arnold Garcia portrait
- `ben-woodall.webp` - Ben Woodall portrait
- `derek-parks.webp` - Derek Parks portrait
- `jennifer-tene.jpg` / `jennifer-tene.webp` - Jennifer Tene portrait variants
- `jeremy-thamert.webp` - Jeremy Thamert portrait
- `lisa.jpg` / `lisa.webp` - Lisa portrait variants
- `matt-ramsey.jpg` / `matt-ramsey.webp` - Matt Ramsey portrait variants
- `mike-holstein.jpg` / `mike-holstein.webp` - Mike Holstein portrait variants
- `porter-cline.webp` - Porter Cline portrait
- `reagan-massey.webp` - Reagan Massey portrait
- `todd-schoeff.webp` - Todd Schoeff portrait
- `mh-construction-team-group-2025.webp` - team group photo

## Tracking Tips

- Keep portrait filenames aligned with the team member slug or display name.
- If a member has both JPG and WebP assets, preserve both when they are intentionally used in different contexts.
- Use the group photo for team-wide layouts and keep it distinct from individual portraits.
