// Navigation item interface
import { SECTION_NAV_ICONS } from "@/lib/constants/navigation-icons";

export interface NavigationItem {
  href: string;
  label: string;
  mobileLabel?: string; // Shorter label for mobile screens
  icon: string;
}

const navigationLabelTranslations: Record<string, string> = {
  Active: "Activo",
  "Active Use": "Uso activo",
  Apply: "Aplicar",
  "Apply Now": "Aplicar ahora",
  Areas: "Areas",
  Awards: "Premios",
  "Awards & Recognition": "Premios y reconocimiento",
  Badges: "Insignias",
  "Become an Ally": "Hazte aliado",
  "Begin Partnership": "Iniciar alianza",
  Benefits: "Beneficios",
  Capabilities: "Capacidades",
  "Career Growth": "Crecimiento profesional",
  "Leadership Team": "Equipo de liderazgo",
  "Project Partners": "Socios de proyecto",
  "Client Stories": "Historias de clientes",
  "Client Testimonials": "Testimonios de clientes",
  Commanders: "Comandantes",
  "Common Questions": "Preguntas comunes",
  "Communication & Support": "Comunicacion y soporte",
  "Company Culture": "Cultura de la empresa",
  "Company Evolution": "Evolucion de la empresa",
  Compliance: "Cumplimiento",
  Comply: "Cumplir",
  "Construction Expertise": "Experiencia en construccion",
  Contact: "Contacto",
  Core: "Base",
  "Core Services": "Servicios principales",
  "Core Values": "Valores centrales",
  Credentials: "Credenciales",
  Creds: "Creds",
  Culture: "Cultura",
  Discount: "Descuento",
  "Employee Stories": "Historias del equipo",
  Evolution: "Evolucion",
  Expertise: "Experiencia",
  FAQ: "FAQ",
  "Field Officers": "Oficiales de campo",
  "Field Ops": "Operaciones de campo",
  Foundation: "Fundamento",
  General: "General",
  "General Information": "Informacion general",
  "Get Started": "Comenzar",
  Gov: "Gob",
  "Government Projects": "Proyectos gubernamentales",
  Growth: "Crecimiento",
  "How to Apply": "Como aplicar",
  "Latest News": "Ultimas noticias",
  Leadership: "Liderazgo",
  "Leave a Review": "Dejar una resena",
  Location: "Ubicacion",
  Logistics: "Logistica",
  "Logistics Command": "Comando logistico",
  "MISH Safety & Health Program": "Programa de Seguridad y Salud MISH",
  "MISH Safety & Health Program (Safety Manual)":
    "Programa de Seguridad y Salud MISH (Manual de seguridad)",
  "Mission Commanders": "Comandantes de mision",
  News: "Noticias",
  "Office Location": "Ubicacion de oficina",
  "Open Positions": "Puestos abiertos",
  Opportunities: "Oportunidades",
  Options: "Opciones",
  "Our Partners": "Nuestros socios",
  "Our Process": "Nuestro proceso",
  "Our Services": "Nuestros servicios",
  "Our Stats": "Nuestras estadisticas",
  Overview: "Resumen",
  Partner: "Socio",
  Partners: "Socios",
  "Partnership Options": "Opciones de alianza",
  "Partnership Paths": "Rutas de alianza",
  Paths: "Rutas",
  Philosophy: "Filosofia",
  Photos: "Fotos",
  Portfolio: "Portafolio",
  Positions: "Puestos",
  Pricing: "Precios",
  "Pricing & Payment": "Precios y pagos",
  Process: "Proceso",
  "Process & Partnership": "Proceso y alianza",
  Program: "Programa",
  "Quick Contact": "Contacto rapido",
  Record: "Registro",
  Review: "Resena",
  Reviews: "Resenas",
  Safety: "Seguridad",
  "Safety & Quality": "Seguridad y calidad",
  "Safety Excellence": "Excelencia en seguridad",
  "Safety Record": "Historial de seguridad",
  "Service Areas": "Areas de servicio",
  Services: "Servicios",
  Snapshots: "Instantaneas",
  "Spec Ops": "Ops especiales",
  "Special Operations": "Operaciones especiales",
  Specialty: "Especialidad",
  "Specialty Services": "Servicios especializados",
  Start: "Inicio",
  Stats: "Estadisticas",
  Stories: "Historias",
  "Strategic Partnerships": "Alianzas estrategicas",
  Support: "Soporte",
  Team: "Equipo",
  "Team Stories": "Historias del equipo",
  Technical: "Tecnico",
  "Technical & PM": "Tecnico y PM",
  Testimonials: "Testimonios",
  "The Upper Brass": "Alto mando",
  Timeline: "Cronologia",
  "Track Record": "Trayectoria",
  Trust: "Confianza",
  "Trust In Action": "Confianza en accion",
  Action: "Accion",
  Values: "Valores",
  "Verified Badges": "Insignias verificadas",
  Veteran: "Veterano",
  "Veteran Benefits": "Beneficios para veteranos",
  "Veteran Discount": "Descuento para veteranos",
  "Veteran Foundation": "Base veterana",
  "Veteran Support": "Apoyo a veteranos",
  "Veteran-Owned": "Empresa veterana",
  Veterans: "Veteranos",
  "Why Choose Us": "Por que elegirnos",
  "Why MHC": "Por que MHC",
  "Why Partner": "Por que asociarse",
  "Why Us": "Por que nosotros",
  "Why Values Matter": "Por que importan los valores",
  "Working Together": "Trabajando juntos",
};

function translateNavigationLabel(label: string, locale: "en" | "es"): string {
  if (locale !== "es") {
    return label;
  }

  return navigationLabelTranslations[label] ?? label;
}

// Helper function to get appropriate label based on screen size
export function getNavigationLabel(
  item: NavigationItem,
  isMobile = false,
  locale: "en" | "es" = "en",
): string {
  const baseLabel =
    isMobile && item.mobileLabel ? item.mobileLabel : item.label;
  return translateNavigationLabel(baseLabel, locale);
}

// Page-specific navigation configurations
export const navigationConfigs = {
  home: [
    {
      href: "/services",
      label: "Services",
      mobileLabel: "Services",
      icon: SECTION_NAV_ICONS.build,
    },
    {
      href: "#why-partner",
      label: "Why Partner",
      mobileLabel: "Why Us",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#ownership-controls",
      label: "Ownership Controls",
      mobileLabel: "Controls",
      icon: SECTION_NAV_ICONS.account_tree,
    },
    {
      href: "#testimonials",
      label: "Testimonials",
      mobileLabel: "Reviews",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#our-process",
      label: "Our Process",
      mobileLabel: "Process",
      icon: SECTION_NAV_ICONS.timeline,
    },
    {
      href: "#next-steps",
      label: "Get Started",
      mobileLabel: "Start",
      icon: SECTION_NAV_ICONS.rocket_launch,
    },
  ],

  about: [
    {
      href: "#partnership-philosophy",
      label: "Philosophy",
      mobileLabel: "Philosophy",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#team",
      label: "Leadership Team",
      mobileLabel: "Team",
      icon: SECTION_NAV_ICONS.military_tech,
    },
    {
      href: "#values",
      label: "Why Values Matter",
      mobileLabel: "Values",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#company-evolution",
      label: "Company Evolution",
      mobileLabel: "Evolution",
      icon: SECTION_NAV_ICONS.timeline,
    },
    {
      href: "#stats",
      label: "Track Record",
      mobileLabel: "Stats",
      icon: SECTION_NAV_ICONS.analytics,
    },
    {
      href: "#awards",
      label: "Awards & Recognition",
      mobileLabel: "Awards",
      icon: SECTION_NAV_ICONS.emoji_events,
    },
    {
      href: "#safety",
      label: "Safety Excellence",
      mobileLabel: "Safety",
      icon: SECTION_NAV_ICONS.shield,
    },
    {
      href: "#news",
      label: "Latest News",
      mobileLabel: "News",
      icon: SECTION_NAV_ICONS.newspaper,
    },
  ],

  services: [
    {
      href: "#core-services",
      label: "Core Services",
      mobileLabel: "Core",
      icon: SECTION_NAV_ICONS.build,
    },
    {
      href: "#specialty",
      label: "Specialty Services",
      mobileLabel: "Specialty",
      icon: SECTION_NAV_ICONS.construction,
    },
    {
      href: "#government",
      label: "Government Projects",
      mobileLabel: "Gov",
      icon: SECTION_NAV_ICONS.account_balance,
    },
    {
      href: "#service-areas",
      label: "Service Areas",
      mobileLabel: "Areas",
      icon: SECTION_NAV_ICONS.map,
    },
    {
      href: "#trust-in-action",
      label: "Trust In Action",
      mobileLabel: "Trust",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#process",
      label: "Our Process",
      mobileLabel: "Process",
      icon: SECTION_NAV_ICONS.timeline,
    },
  ],

  coolDesertNights: [
    {
      href: "#event-overview",
      label: "Overview",
      mobileLabel: "Overview",
      icon: SECTION_NAV_ICONS.event,
    },
    {
      href: "#community-leadership",
      label: "Leadership",
      mobileLabel: "Lead",
      icon: SECTION_NAV_ICONS.groups,
    },
    {
      href: "#event-media",
      label: "Media",
      mobileLabel: "Media",
      icon: SECTION_NAV_ICONS.videocam,
    },
    {
      href: "#event-timeline",
      label: "Timeline",
      mobileLabel: "Timeline",
      icon: SECTION_NAV_ICONS.timeline,
    },
    {
      href: "#event-action",
      label: "Action",
      mobileLabel: "Action",
      icon: SECTION_NAV_ICONS.rocket_launch,
    },
  ],

  testimonials: [
    {
      href: "#client-testimonials",
      label: "Client Testimonials",
      mobileLabel: "Reviews",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#why-choose",
      label: "Why Choose Us",
      mobileLabel: "Why Us",
      icon: SECTION_NAV_ICONS.star,
    },
    {
      href: "#trust-signals",
      label: "Credentials",
      mobileLabel: "Creds",
      icon: SECTION_NAV_ICONS.workspace_premium,
    },
    {
      href: "#leave-review",
      label: "Leave a Review",
      mobileLabel: "Review",
      icon: SECTION_NAV_ICONS.rate_review,
    },
  ],

  faq: [
    {
      href: "#general",
      label: "General Information",
      mobileLabel: "General",
      icon: SECTION_NAV_ICONS.info,
    },
    {
      href: "#process",
      label: "Process & Partnership",
      mobileLabel: "Process",
      icon: SECTION_NAV_ICONS.timeline,
    },
    {
      href: "#safety",
      label: "Safety & Quality",
      mobileLabel: "Safety",
      icon: SECTION_NAV_ICONS.shield,
    },
    {
      href: "#communication",
      label: "Communication & Support",
      mobileLabel: "Support",
      icon: SECTION_NAV_ICONS.campaign,
    },
    {
      href: "#veterans",
      label: "Veteran Benefits",
      mobileLabel: "Veterans",
      icon: SECTION_NAV_ICONS.military_tech,
    },
    {
      href: "#technical",
      label: "Technical & PM",
      mobileLabel: "Technical",
      icon: SECTION_NAV_ICONS.engineering,
    },
    {
      href: "#partnership",
      label: "Working Together",
      mobileLabel: "Partners",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#financial",
      label: "Pricing & Payment",
      mobileLabel: "Pricing",
      icon: SECTION_NAV_ICONS.payments,
    },
  ],

  projects: [
    {
      href: "#portfolio",
      label: "Trust In Action",
      mobileLabel: "Portfolio",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#capabilities",
      label: "Capabilities",
      mobileLabel: "Capabilities",
      icon: SECTION_NAV_ICONS.engineering,
    },
    {
      href: "#Veteran-Owned",
      label: "Veteran-Owned",
      mobileLabel: "Veteran",
      icon: SECTION_NAV_ICONS.military_tech,
    },
    {
      href: "#stats",
      label: "Our Stats",
      mobileLabel: "Stats",
      icon: SECTION_NAV_ICONS.analytics,
    },
    {
      href: "#testimonials",
      label: "Client Stories",
      mobileLabel: "Stories",
      icon: SECTION_NAV_ICONS.star,
    },
  ],

  team: [
    {
      href: "#employee-testimonials",
      label: "Team Stories",
      mobileLabel: "Stories",
      icon: SECTION_NAV_ICONS.star,
    },
    {
      href: "#company-culture",
      label: "Company Culture",
      mobileLabel: "Culture",
      icon: SECTION_NAV_ICONS.diversity_3,
    },
    {
      href: "#career-growth",
      label: "Career Growth",
      mobileLabel: "Growth",
      icon: SECTION_NAV_ICONS.school,
    },
    {
      href: "#founder-tribute",
      label: "Founder Tribute",
      mobileLabel: "Founder",
      icon: SECTION_NAV_ICONS.foundation,
    },
  ],

  careers: [
    {
      href: "#positions",
      label: "Open Positions",
      mobileLabel: "Positions",
      icon: SECTION_NAV_ICONS.work,
    },
    {
      href: "#application-process",
      label: "How to Apply",
      mobileLabel: "Apply",
      icon: SECTION_NAV_ICONS.timeline,
    },
    {
      href: "#testimonials",
      label: "Employee Stories",
      mobileLabel: "Stories",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#general-application",
      label: "Apply Now",
      mobileLabel: "Apply",
      icon: SECTION_NAV_ICONS.handshake,
    },
  ],

  contact: [
    {
      href: "#quick-contact",
      label: "Quick Contact",
      mobileLabel: "Contact",
      icon: SECTION_NAV_ICONS.forum,
    },
    {
      href: "#partnership-pathways-heading",
      label: "Partnership Paths",
      mobileLabel: "Paths",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#office-location-heading",
      label: "Office Location",
      mobileLabel: "Location",
      icon: SECTION_NAV_ICONS.location_on,
    },
    {
      href: "#partnership-options-heading",
      label: "Partnership Options",
      mobileLabel: "Options",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#service-areas",
      label: "Service Areas",
      mobileLabel: "Areas",
      icon: SECTION_NAV_ICONS.map,
    },
  ],

  publicSector: [
    {
      href: "#overview",
      label: "Overview",
      mobileLabel: "Overview",
      icon: SECTION_NAV_ICONS.info,
    },
    {
      href: "/services",
      label: "Our Services",
      mobileLabel: "Services",
      icon: SECTION_NAV_ICONS.build,
    },
    {
      href: "#contact",
      label: "Get Started",
      mobileLabel: "Start",
      icon: SECTION_NAV_ICONS.contact_phone,
    },
  ],

  allies: [
    {
      href: "#vendors",
      label: "Our Partners",
      mobileLabel: "Partners",
      icon: SECTION_NAV_ICONS.groups,
    },
    {
      href: "#partnership",
      label: "Why MHC",
      mobileLabel: "Why MHC",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#apply",
      label: "Become an Ally",
      mobileLabel: "Apply",
      icon: SECTION_NAV_ICONS.diversity_3,
    },
  ],

  tradePartners: [
    {
      href: "#overview",
      label: "Overview",
      mobileLabel: "Overview",
      icon: SECTION_NAV_ICONS.info,
    },
    {
      href: "#benefits",
      label: "Benefits",
      mobileLabel: "Benefits",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#opportunities",
      label: "Opportunities",
      mobileLabel: "Opportunities",
      icon: SECTION_NAV_ICONS.handshake,
    },
    {
      href: "#contact",
      label: "Begin Partnership",
      mobileLabel: "Partner",
      icon: SECTION_NAV_ICONS.diversity_3,
    },
  ],

  veterans: [
    {
      href: "#veteran-leadership",
      label: "Veteran Foundation",
      mobileLabel: "Foundation",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#combat-veteran-discount",
      label: "Veteran Discount",
      mobileLabel: "Discount",
      icon: SECTION_NAV_ICONS.military_tech,
    },
    {
      href: "#year-round-support",
      label: "Veteran Support",
      mobileLabel: "Support",
      icon: SECTION_NAV_ICONS.volunteer_activism,
    },
    {
      href: "#veteran-partnerships",
      label: "Strategic Partnerships",
      mobileLabel: "Partners",
      icon: SECTION_NAV_ICONS.groups,
    },
  ],

  safety: [
    {
      href: "#credentials",
      label: "Credentials",
      mobileLabel: "Creds",
      icon: SECTION_NAV_ICONS.workspace_premium,
    },
    {
      href: "#credential-badges",
      label: "Verified Badges",
      mobileLabel: "Badges",
      icon: SECTION_NAV_ICONS.verified,
    },
    {
      href: "#program",
      label: "MISH Safety & Health Program (Safety Manual)",
      mobileLabel: "Program",
      icon: SECTION_NAV_ICONS.menu_book,
    },
    {
      href: "#performance",
      label: "Safety Record",
      mobileLabel: "Record",
      icon: SECTION_NAV_ICONS.shield,
    },
    {
      href: "#evidence",
      label: "Active Use",
      mobileLabel: "Active",
      icon: SECTION_NAV_ICONS.fact_check,
    },
    {
      href: "#compliance",
      label: "Compliance",
      mobileLabel: "Comply",
      icon: SECTION_NAV_ICONS.gpp_good,
    },
    {
      href: "#snapshots",
      label: "Snapshots",
      mobileLabel: "Photos",
      icon: SECTION_NAV_ICONS.photo_camera,
    },
  ],

  events: [
    {
      href: "#yearly-hosted-event",
      label: "Annual Hosted Event",
      mobileLabel: "Annual",
      icon: SECTION_NAV_ICONS.event,
    },
    {
      href: "#fishing-event-ribbon",
      label: "Fishing Event Ribbon",
      mobileLabel: "Fishing",
      icon: SECTION_NAV_ICONS.emoji_events,
    },
    {
      href: "#event-gallery",
      label: "Event Gallery",
      mobileLabel: "Gallery",
      icon: SECTION_NAV_ICONS.photo_library,
    },
    {
      href: "#upcoming-events",
      label: "Upcoming Events",
      mobileLabel: "Upcoming",
      icon: SECTION_NAV_ICONS.calendar_month,
    },
    {
      href: "/contact",
      label: "Partner With MH",
      mobileLabel: "Partner",
      icon: SECTION_NAV_ICONS.groups,
    },
  ],

  resources: [
    {
      href: "#manuals",
      label: "Safety Manual",
      mobileLabel: "Manual",
      icon: SECTION_NAV_ICONS.menu_book,
    },
    {
      href: "#forms",
      label: "Forms & Toolbox Talks",
      mobileLabel: "Forms",
      icon: SECTION_NAV_ICONS.description,
    },
    {
      href: "#downloads",
      label: "Field Downloads",
      mobileLabel: "Downloads",
      icon: SECTION_NAV_ICONS.cloud_download,
    },
    {
      href: "/services",
      label: "Our Services",
      mobileLabel: "Services",
      icon: SECTION_NAV_ICONS.build,
    },
  ],

  locations: [
    {
      href: "#offices",
      label: "Office Locations",
      mobileLabel: "Offices",
      icon: SECTION_NAV_ICONS.location_on,
    },
    {
      href: "#service-areas",
      label: "Service Areas",
      mobileLabel: "Areas",
      icon: SECTION_NAV_ICONS.public,
    },
    {
      href: "#contact-info",
      label: "Contact Info",
      mobileLabel: "Contact",
      icon: SECTION_NAV_ICONS.phone,
    },
    {
      href: "/contact",
      label: "Get In Touch",
      mobileLabel: "Connect",
      icon: SECTION_NAV_ICONS.mail,
    },
  ],
};

export type PageType = keyof typeof navigationConfigs;
