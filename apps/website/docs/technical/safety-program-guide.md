...
│   ├── employee-handbook.json                            ← Employee Handbook manifest (sections array with docxPath refs)
...
│   └── MHC-Employee-Handbook-Sections/                   ← Handbook section DOCX files (auto-generated by dissect script)
├── forms/
...
│   ├── dissect-employee-handbook.mjs  ← Creates 6 handbook section DOCX files from handbook structure
...
### Handbook Dissection Workflow

The Employee Handbook PDF is dissected into structured sections for pipeline consistency:
...
2. **Update handbook manifest:**
   The dissect script populates `documents/content/employee-handbook.json` with section metadata
   (title, subtitle, docxPath, page ranges).
