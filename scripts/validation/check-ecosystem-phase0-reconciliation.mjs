...
requireControl(register.schemaVersion === 1, 'Phase 0 register schema must be Version 1.');
requireControl(register.lifecycleStatus === 'Rough Draft', 'Phase 0 lifecycle must remain Rough Draft.');
requireControl(records.length === 193, `Expected 193 controlled records; found ${records.length}.`);
requireControl(new Set(recordIds).size === 193, 'Controlled record IDs must be unique.');
requireControl(register.sealedArtifact.docxCount === 193, 'Sealed DOCX count must be 193.');
requireControl(register.sealedArtifact.pdfCount === 193, 'Sealed PDF count must be 193.');
requireControl(
  register.sealedArtifact.renderedPageCount === 1008,
  'Sealed rendered-page count must be 1,008.',
);
requireControl(register.sealedArtifact.numberedPackageCount === 10, 'Package count must be ten.');
requireControl(register.sealedArtifact.companyBibleSeparate, 'Company Bible must remain separately controlled.');
requireControl(register.repositoryIntake.docxCount === 191, 'Repository intake must report 191 DOCX files.');
requireControl(
...
requireControl(
  records.every((record) => allowedDispositions.has(record.repositoryDisposition)),
  'Every controlled record must have an allowed Phase 0 disposition.',
);
requireControl(
  records.every((record) => record.publicationClassification === 'not-public-draft'),
  'Every Rough Draft record must remain classified as not public.',
);
requireControl(
  !records.some((record) => record.repositoryDisposition === 'unexplained-repository-omission'),
  'Phase 0 contains an unexplained repository omission.',
);
requireControl(
  register.dispositionCounts['repository-working-copy-diverged-from-sealed-baseline'] === 98,
...
requireControl(
  authority.websiteIntegration.currentGate === 'phase-1-authority-pipeline-foundation',
...
requireControl(!register.controls.publicDraftDownloadsAllowed, 'Draft public downloads must remain blocked.');
requireControl(!register.controls.fieldEffective, 'Rough Draft records cannot be field-effective.');
