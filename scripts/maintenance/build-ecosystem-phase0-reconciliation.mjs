...
for (const file of repositoryFiles) {
  ...
  if (repositoryByKey.has(key))
    throw new Error(`Duplicate repository document key: ${key}`);
...
for (const source of metadata) {
  const repositoryFamily =
    source.package === "company-bible" ? "01-core-doctrine" : source.package;
...
const dispositionCounts = Object.fromEntries(
  [...new Set(records.map((record) => record.repositoryDisposition))]
    ...
    .map((disposition) => [
      disposition,
      records.filter((record) => record.repositoryDisposition === disposition)
        .length,
    ]),
);
const packageCounts = Object.fromEntries(
  [...new Set(records.map((record) => record.package))]
    ...
    .map((family) => [
      family,
      records.filter((record) => record.package === family).length,
    ]),
);
...
const reconciliation = {
  ...
  sealedArtifact: {
    ...
    docxCount: 193,
    pdfCount: 193,
    renderedPageCount: records.reduce(
      (total, record) => total + record.pdfPhysicalPages,
      0,
    ),
