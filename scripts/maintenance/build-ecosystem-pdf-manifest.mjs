...
for (const [index, record] of reconciliation.records.entries()) {
  ...
  const sourceHash = sha256(
    await readFile(path.join(repositoryRoot, sourcePdf)),
  );
  if (sourceHash !== record.pdfSha256)
    throw new Error(`PDF hash mismatch: ${sourcePdf}`);

  const family =
    record.package === "06-tbt-library"
      ? "tbt"
      : record.package === "07-sds-library"
        ? "sds"
        : "ecosystem";
  const isForm =
    /^(form-|ehb-f-|eom-f-|fin-f-|pep-f-|sub-f-|wmg-f-|bid-f-)/i.test(
      record.documentId,
    );
  ...
  const printPageCount =
    record.pdfPhysicalPages - (omitControlCoverInPrint ? 1 : 0);
  ...
  entries.push({
    ...
    includeInMasterNotebook:
      record.distributionType !== "authorized-distribution-copy",
    ...
    treatment: {
      class: treatment,
      individualCover:
        treatment === "controlled-publication" ||
        treatment === "governing-publication",
      individualToc:
        (treatment === "controlled-publication" ||
          treatment === "governing-publication") &&
...
const manifest = {
  ...
  binderPlan: {
    ...
    masterNotebookRecordCount: entries.filter(
  ...
  counts: {
    ...
    printForms: entries.filter(
      (entry) => entry.treatment.class === "print-form",
    ).length,
  },
  entries,
};
...
console.log(
  `Ecosystem PDF manifest generated: ${entries.length} verified PDFs across ten packages plus the Company Bible.`,
);
