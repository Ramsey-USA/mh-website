...
const expectedOutputSuffix = path.join(
  "apps",
  "website",
  "documents",
  "generated-pdfs",
...
if (
  !path.resolve(outputRoot).endsWith(expectedOutputSuffix) ||
  path.resolve(outputRoot) === path.resolve(repositoryRoot)
) {
...
function displayTitle(entry) {
  return entry.documentId
    ...
    .split("-")
    .map((word) =>
      ["mh", "mish", "sds", "tbt", "it", "ai", "jha"].includes(word)
        ? word.toUpperCase()
...
async function addToc(document, fonts, tocEntries, title) {
  ...
  for (let offset = 0; offset < tocEntries.length; offset += linesPerPage) {
    ...
    tocEntries.slice(offset, offset + linesPerPage).forEach((entry, index) => {
      ...
      const titleText =
        entry.title.length > 69
          ? `${entry.title.slice(0, 66)}...`
          : entry.title;
...
function addDivider(document, fonts, packageName) {
  ...
  page.drawText(
    packageName === "company-bible" ? "GOVERNING" : packageName.slice(0, 2),
    {
      x: 164,
      y: 610,
      size: 16,
  ...
  page.drawText("CONTROLLED SECTION TAB", {
    x: 164,
    y: 485,
    size: 11,
    font: fonts.regular,
...
async function addSourceEntries(document, entries, fonts) {
  for (const entry of entries) {
    ...
    if (sha256(source) !== entry.pdfSha256)
      throw new Error(`PDF hash mismatch: ${entry.sourcePdf}`);
...
const packageNames = [
  ...new Set(
    manifest.entries
      .filter((entry) => entry.includeInPackageBinder)
...
const masterEntries = manifest.entries.filter(
  (entry) => entry.includeInMasterNotebook,
);
const masterGroups = [
  {
    package: "company-bible",
    entries: masterEntries.filter((entry) => entry.package === "company-bible"),
  },
...
console.log(
  `Built ${packageNames.length} package binders, printable spines, and one ${masterEntries.length}-record master Ecosystem notebook.`,
);
