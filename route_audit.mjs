...
async function audit() {
  ...
  try {
    ...
    console.error("Error reading all_pages.txt:", e.message);
  }
  ...
  try {
    ...
    [...faqData.matchAll(/id:\s*["']([^"']+)["']/g)].forEach((m) =>
      routes.add(`/faq/${m[1]}`),
    );
  } catch (e) {}
  ...
  try {
    ...
    [...serviceData.matchAll(/slug:\s*["']([^"']+)["']/g)].forEach((m) =>
  ...
  try {
    ...
    [...projectData.matchAll(/slug:\s*["']([^"']+)["']/g)].forEach((m) =>
      routes.add(`/projects/${m[1]}`),
    );
  } catch (e) {}
  ...
  try {
    ...
    const match = locationsData.match(
      /export const locations: Record<[^>]+> = ({[\s\S]+?});/,
    );
    if (match) {
      [...match[1].matchAll(/^\s{2}['"]?([\w-]+)['"]?:/gm)].forEach((m) =>
        routes.add(`/locations/${m[1]}`),
      );
    }
  } catch (e) {}

  try {
    const safetyClusters = fs.readFileSync(
      "apps/website/src/lib/data/safety-manual-clusters.ts",
      "utf8",
    );
    [...safetyClusters.matchAll(/slug:\s*["']([^"']+)["']/g)].forEach((m) =>
      routes.add(`/resources/safety-manual/${m[1]}`),
    );
  } catch (e) {}

  try {
    const safetySections = JSON.parse(
      fs.readFileSync("documents/content/safety-manual-public.json", "utf8"),
    );
    safetySections.sections.forEach((s) =>
      routes.add(`/resources/safety-manual/section/${s.slug}`),
    );
  } catch (e) {}
  ...
  for (const route of routeList) {
    try {
      ...
      results.failures.push(`${route} (Error: ${e.message})`);
    }
  }
  ...
  console.log(
    `Runtime Signatures (${results.runtimeErrors.length}):`,
