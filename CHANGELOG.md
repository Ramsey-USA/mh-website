...
- **Jul 24 (b):** Closed Cool Desert Nights canonical-overlap gap by consolidating canonical ownership to `/events/cool-desert-nights` and preserving `/cool-desert-nights` as a permanent legacy redirect for link continuity. Updated route metadata behavior, redirect registry, canonical route-manifest coverage, locale-switcher approved path list, and Events documentation gap status/verification notes.

- **Jul 24:** Events hub four-page rollout documentation and SEO/branding progression tracking — updated event surfaces and governance records to reflect four dedicated event routes (`/events/operation-cast-recover`, `/events/cool-desert-nights`, `/events/bbq-contest`, `/events/ironman-volunteer`) with route-specific Jeremy ribbon coverage, per-page metadata/schema refinements, and dedicated events documentation records. Added event-progress references in `docs/events/index.md` and `docs/business/index.md`, and introduced two new dedicated event records: `docs/events/bbq-contest-planning-record.md` and `docs/events/ironman-volunteer-community-involvement.md`. Logical-gap tracking now explicitly documents (1) legacy `/cool-desert-nights` vs dedicated `/events/cool-desert-nights` overlap, (2) pending slug/title transition for BBQ naming finalization, and (3) external IRONMAN volunteer signup analytics visibility limitations.

- **Jul 22 (h):** Company-wide hero commercial pipeline activation and homepage integration - activated the first approved company campaign (`Command the Horizon`) with canonical single-partner media registration (`mhc-command-the-horizon-2026q3-v01`, Stephens Media Group), generated and stored compliant MP4/WebM/poster assets under `apps/website/public/videos/hero-commercials/`, and wrote the active manifest entry in `apps/website/config/hero-commercials.json` including Jeremy Thamert voice/presenter SEO fields plus company route-application mapping. Extended validation/generator workflows to support company-vs-route campaign scope, strict single-partner enforcement (`smg` or `tsm`, never both), and synchronized homepage runtime selection so `apps/website/src/components/home/HeroSection.tsx` now resolves manifest-backed hero video playback via `HeroSectionClient` with static poster fallback. Updated docs across standards/media/homepage/performance to reflect active production state and operational workflow.

- **Jul 22:** Dependabot high-alert remediation for development dependency graph (alerts #93, #94, #95, #96, #98) — resolved transitive vulnerabilities in `brace-expansion`, `js-yaml`, and `fast-uri` by updating workspace-level pnpm override policy and regenerating `pnpm-lock.yaml` with targeted patched resolutions (`brace-expansion` 1.1.16/2.1.2, `js-yaml` 4.3.0, `fast-uri` 3.1.4). Root-level override guardrails were also aligned for consistency. Validation evidence: `pnpm audit --prod --json` PASS (`high=0`, `critical=0`) and `pnpm audit --dev --json` PASS (`high=0`, `critical=0`).
