...
// ── Manual section data (JSON-driven) ────────────────────────────────────────

/** Canonical typed view of a section in `documents/content/safety-manual.json`. */
...
const manualJson = safetyManualJson as unknown as ManualJsonShape;

/** All 50 canonical MISH sections, in order (number 1–50). */
export const MANUAL_SECTIONS: ReadonlyArray<ManualSection> =
  manualJson.sections;

/** Lookup a section by canonical slug. */
export function sectionBySlug(slug: string): ManualSection | undefined {
  return MANUAL_SECTIONS.find((s) => s.slug === slug);
}

/** Lookup a section by MISH number (1-50). */
export function sectionByNumber(n: number): ManualSection | undefined {
  return MANUAL_SECTIONS.find((s) => s.number === n);
}
