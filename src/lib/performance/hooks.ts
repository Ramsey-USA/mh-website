...
import { performanceManager, queryOptimizer, type PerformanceMetric } from './performance-manager'
...
export function useRenderTiming(componentName: string, dependencies?: React.DependencyList) {
...
export function useMemoryMonitoring(componentName: string, interval: number = 10000) {
...
export function usePerformanceMetrics(refreshInterval: number = 5000) {
  ...
  const [report, setReport] = useState<ReturnType<typeof performanceManager.getPerformanceReport> | null>(null)
...
export function useDynamicImport<T>(
  ...
  useEffect(() => {
    ...
    importFn()
      ...
      .catch((err) => {
        setError(err instanceof Error ? err : new Error('Dynamic import failed'))
        performanceManager.endTiming(`dynamic_import_${componentName}`)
      })
      .finally(() => {
        setLoading(false)
