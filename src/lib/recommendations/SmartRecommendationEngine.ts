...
import type { MilitaryConstructionAI } from '../militaryConstructionAI'
...
export class SmartRecommendationEngine {
  ...
  constructor(aiEngine?: MilitaryConstructionAI, enableABTesting: boolean = true) {
  ...
  private analyzeUrgency(timeframe: string): 'immediate' | 'soon' | 'planned' {
    if (timeframe.includes('immediately') || timeframe.includes('asap'))
      return 'immediate'
    if (timeframe.includes('month') || timeframe.includes('soon')) return 'soon'
    return 'planned'
  }
