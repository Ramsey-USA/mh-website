import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Contact MH Construction - Get a Free Quote | (509) 308-6489",
  description: "Contact MH Construction for a free consultation. Serving Tri-Cities (Pasco, Kennewick, Richland) and Pacific Northwest. Call (509) 308-6489 or email info@mhc-gc.com. Veteran-owned excellence.",
  keywords: [
    "contact MH Construction",
    "construction quote",
    "free construction consultation",
    "Tri-Cities contractor contact",
    "Pasco construction phone",
    "(509) 308-6489",
    "construction company email",
    "get construction quote",
    "contact contractor Tri-Cities",
    "veteran-owned contractor contact",
  ],
  openGraph: {
    title: "Contact MH Construction - Free Consultation",
    description: "Get in touch for a free consultation. Serving the Tri-Cities with veteran-owned construction excellence. Call (509) 308-6489.",
    type: "website",
    url: "https://www.mhc-gc.com/contact",
  },
  alternates: {
    canonical: "https://www.mhc-gc.com/contact",
  },
};
