...
describe("SemiquincentennialBanner", () => {
  ...
  it("unmounts cleanly when no animation is running (no error thrown)", () => {
    const { unmount } = render(<SemiquincentennialBanner />);
    expect(() => unmount()).not.toThrow();
  });

  it("cancels animation frame on unmount when one is in progress", () => {
    const cancelSpy = jest
      .spyOn(window, "cancelAnimationFrame")
      .mockImplementation(() => {});

    // Provide a requestAnimationFrame that returns a handle
    jest
      .spyOn(window, "requestAnimationFrame")
      .mockImplementation((cb: FrameRequestCallback) => {
        // Call the callback immediately so the loop runs once and sets the ref
        setTimeout(() => cb(performance.now()), 0);
        return 42;
      });

    // getContext returns a real-enough mock for the canvas path to run
    const ctxMock = {
      clearRect: jest.fn(),
    ...
    // Click Celebrate! to start an animation
    const btn = screen.getByRole("button", { name: /Launch fireworks/i });
    fireEvent.click(btn);

    unmount();

    // cancelAnimationFrame should have been called on unmount cleanup
    expect(cancelSpy).toHaveBeenCalled();
  });

  it("clicking Celebrate! twice cancels the first animation before starting a new one", () => {
    const cancelSpy = jest
      .spyOn(window, "cancelAnimationFrame")
      .mockImplementation(() => {});

    // Return a fixed handle so animFrameRef.current is set after first click
    jest.spyOn(window, "requestAnimationFrame").mockReturnValue(99);

    // Provide a minimal ctx so launchFireworks doesn't early-return on getContext
    const ctxMock = {
      clearRect: jest.fn(),
    ...
    // First click — starts animation; animFrameRef.current = 99
    fireEvent.click(btn);
    // Second click — should cancel handle 99 before starting new
    fireEvent.click(btn);
